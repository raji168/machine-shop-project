...
export class MachineMappingDataService {
    ...
    updateMachineMap(machineMapResponse: MachineMapping) {
        const updateMachineMap = this.machineMaps.find(machineMap => machineMap._id === machineMapResponse._id)
        const updateMachineMapIndex = this.machineMaps.findIndex(machineMap => machineMap._id === machineMapResponse._id)
    ...
    deleteMachineMap(id: string) {
        this.machineMaps = this.machineMaps.filter(machineMap => machineMap._id !== id);
        this.machineMapUpdated$.next(this.machineMaps);
